...
class TestImproveCore(unittest.TestCase):
    ...
    def test_core_scan_runs_all_10_tools(self) -> None:
        ...
        for script in expected_tools:
            self.assertTrue(
                (tool_path / script).exists(),
                f"Core-scan tool missing: {script}"
            )
    ...
    def test_core_scan_dispatches_all_tools(self) -> None:
        ...
        expected = [
            "multiscan.py", "complexity.py", "depgraph.py",
            "security_scan.py", "env_check.py", "project_doctor.py",
            "todo_tracker.py", "dead_code_finder.py",
            "dependency_audit.py", "license_checker.py",
        ]
        self.assertEqual(
            len(tool_scripts), 10,
            f"Expected 10 tools, found {len(tool_scripts)}: {tool_scripts}"
        )
        for script in expected:
            self.assertIn(script, tool_scripts,
    ...
    def test_no_shell_true_in_source(self) -> None:
        ...
        allowlist = {
            "safe_run.py", "command_guard.py", "release_packager.py",
            "test_command_guard.py", "test_safe_run.py",
        }
        violations = []
                ...
                if isinstance(node, ast.Call):
                    for kw in getattr(node, 'keywords', []) or []:
                        if kw.arg == 'shell' and isinstance(kw.value, ast.Constant) and kw.value.value is True:
                            violations.append(f"{py_file.relative_to(project_root)}: subprocess call with shell=True")
                # Check os.system() calls
                if isinstance(node, ast.Call):
                    if isinstance(node.func, ast.Attribute):
                        if node.func.attr == 'system' and isinstance(node.func.value, ast.Name) and node.func.value.id == 'os':
                            violations.append(f"{py_file.relative_to(project_root)}: os.system() call")
        if violations:
            self.fail("Direct shell=True or os.system() found in source (use safe_run instead):\n" + "\n".join(violations))
        else:
